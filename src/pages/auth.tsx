...
const Auth = () => {
	...
	const readonlyInput = () => {
		isLoading ? setReadonly(true) : setReadonly(false)
	}
			...
			<Formik initialValues={{ email: '', password: '' }} onSubmit={onSubmit} validationSchema={validation}>
				<Form className='relative mt-24 space-y-8 rounded bg-black/75 py-10 px-6 md:mt-0 md:max-w-md md:px-10'>
					...
					<div className='space-y-4'>
						<TextField name='email' placeholder='Email' type={'text'} readOnly={readonly} />
						<TextField name='password' placeholder='Password' type={'password'} readOnly={readonly} />
					</div>

					<button onClick={readonlyInput} type='submit' disabled={isLoading} className='w-full bg-[#E10856] py-4 rounded mt-4 font-semibold'>
